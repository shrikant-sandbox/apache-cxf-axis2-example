package com.svashishtha.ws;

import java.net.MalformedURLException;
import java.net.URL;
import javax.xml.namespace.QName;
import javax.xml.ws.WebEndpoint;
import javax.xml.ws.WebServiceClient;
import javax.xml.ws.WebServiceFeature;
import javax.xml.ws.Service;

/**
 * This class was generated by Apache CXF 2.6.2
 * 2012-12-31T13:22:43.400+08:00
 * Generated source version: 2.6.2
 * 
 */
@WebServiceClient(name = "WrappedService", 
                  wsdlLocation = "src/main/resources/SimpleService.wsdl",
                  targetNamespace = "http://svashishtha.com/ws") 
public class WrappedService_Service extends Service {

    public final static URL WSDL_LOCATION;

    public final static QName SERVICE = new QName("http://svashishtha.com/ws", "WrappedService");
    public final static QName P1 = new QName("http://svashishtha.com/ws", "p1");
    static {
        URL url = WrappedService_Service.class.getResource("src/main/resources/SimpleService.wsdl");
        if (url == null) {
            java.util.logging.Logger.getLogger(WrappedService_Service.class.getName())
                .log(java.util.logging.Level.INFO, 
                     "Can not initialize the default wsdl from {0}", "src/main/resources/SimpleService.wsdl");
        }       
        WSDL_LOCATION = url;
    }

    public WrappedService_Service(URL wsdlLocation) {
        super(wsdlLocation, SERVICE);
    }

    public WrappedService_Service(URL wsdlLocation, QName serviceName) {
        super(wsdlLocation, serviceName);
    }

    public WrappedService_Service() {
        super(WSDL_LOCATION, SERVICE);
    }
    
    //This constructor requires JAX-WS API 2.2. You will need to endorse the 2.2
    //API jar or re-run wsdl2java with "-frontend jaxws21" to generate JAX-WS 2.1
    //compliant code instead.
    public WrappedService_Service(WebServiceFeature ... features) {
        super(WSDL_LOCATION, SERVICE, features);
    }

    //This constructor requires JAX-WS API 2.2. You will need to endorse the 2.2
    //API jar or re-run wsdl2java with "-frontend jaxws21" to generate JAX-WS 2.1
    //compliant code instead.
    public WrappedService_Service(URL wsdlLocation, WebServiceFeature ... features) {
        super(wsdlLocation, SERVICE, features);
    }

    //This constructor requires JAX-WS API 2.2. You will need to endorse the 2.2
    //API jar or re-run wsdl2java with "-frontend jaxws21" to generate JAX-WS 2.1
    //compliant code instead.
    public WrappedService_Service(URL wsdlLocation, QName serviceName, WebServiceFeature ... features) {
        super(wsdlLocation, serviceName, features);
    }

    /**
     *
     * @return
     *     returns WrappedService
     */
    @WebEndpoint(name = "p1")
    public WrappedService getP1() {
        return super.getPort(P1, WrappedService.class);
    }

    /**
     * 
     * @param features
     *     A list of {@link javax.xml.ws.WebServiceFeature} to configure on the proxy.  Supported features not in the <code>features</code> parameter will have their default values.
     * @return
     *     returns WrappedService
     */
    @WebEndpoint(name = "p1")
    public WrappedService getP1(WebServiceFeature... features) {
        return super.getPort(P1, WrappedService.class, features);
    }

}
